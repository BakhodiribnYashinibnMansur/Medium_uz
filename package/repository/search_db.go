...
func (repo *SearchDB) SearchPost(search string, pagination model.Pagination, logrus *logrus.Logger) ([]model.PostFull, error) {
	...
	query := fmt.Sprintf("SELECT id , post_title ,post_image_path, post_body, post_views_count, post_like_count, post_rated, post_vote, post_tags,  post_date, is_new, is_top_read FROM %s WHERE post_title  ~*  $1  AND deleted_at IS NULL  OFFSET $2 LIMIT $3", postTable)
...
func (repo *SearchDB) SearchUser(search string, pagination model.Pagination, logrus *logrus.Logger) ([]model.UserFull, error) {
	...
	query := fmt.Sprintf("SELECT  	id,	email,	firstname,secondname,nickname,		city,	is_verified,	bio,interests,account_image_path,	phone,	rating,	post_views_count,	follower_count, following_count,like_count,is_super_user	FROM %s WHERE firstname ~* $1 OR secondname ~* $2 AND deleted_at IS NULL  OFFSET $2 LIMIT $3 ", usersTable)
