...
func (repo *PostDB) GetResentPost(pagination model.Pagination, logrus *logrus.Logger) (posts []model.PostFull, err error) {
	...
	query := fmt.Sprintf("SELECT p.id , p.post_title ,p.post_image_path, p.post_views_count, p.post_like_count, p.post_rated, p.post_vote_count, p.post_tags, p. post_date, p.is_new, p.is_top_read,pu.post_author_id ,u.firstname,u.secondname,u.account_image_path,u.nickname FROM %s p INNER JOIN %s pu on p.id =pu.post_id INNER JOIN %s u ON u.id= pu.post_author_id WHERE  pu.deleted_at IS NULL AND p.deleted_at IS NULL AND p.deleted_at IS NULL ORDER BY p.post_date DESC OFFSET $1 LIMIT $2 ", postTable, postUserTable, usersTable)
...
func (repo *PostDB) GetMostViewed(pagination model.Pagination, logrus *logrus.Logger) (posts []model.PostFull, err error) {
	...
	query := fmt.Sprintf("SELECT p.id , p.post_title ,p.post_image_path, p.post_views_count, p.post_like_count, p.post_rated, p.post_vote_count, p.post_tags, p. post_date, p.is_new, p.is_top_read,pu.post_author_id ,u.firstname,u.secondname,u.account_image_path,u.nickname FROM %s p INNER JOIN %s pu on p.id =pu.post_id INNER JOIN %s u ON u.id= pu.post_author_id  WHERE  pu.deleted_at IS NULL AND p.deleted_at IS NULL ORDER BY p.post_views_count DESC OFFSET $1 LIMIT $2 ", postTable, postUserTable, usersTable)
	err = repo.db.Select(&posts, query, pagination.Offset, pagination.Limit)
	if err != nil {
		logrus.Errorf("ERROR: don't get users %s", err)
		return posts, err
