...
func (repo *PostDB) GetPostByIdWithoutBody(id int, logrus *logrus.Logger) (post model.PostFull, err error) {

	query := fmt.Sprintf("SELECT p.id , p.post_title ,p.post_image_path, p.post_views_count, p.post_like_count, p.post_rated, p.post_vote_count, p.post_tags, p. post_date, p.is_new, p.is_top_read,pu.post_author_id FROM %s p INNER JOIN %s pu on p.id =pu.post_id WHERE pu.post_id = $1 AND pu.deleted_at IS NULL AND p.deleted_at IS NULL", postTable, postUserTable)
...
func (repo *PostDB) GetPostBodyById(id int, logrus *logrus.Logger) (post model.PostFull, err error) {
	...
	query := fmt.Sprintf("SELECT id , post_body FROM %s WHERE id = $1 AND deleted_at IS NULL  ", postTable)
	err = repo.db.Get(&post, query, id)
	if err != nil {
		if err == sql.ErrNoRows {
			return post, errors.New("ID not found")
		}
		logrus.Errorf("ERROR: don't get post body %s", err)
		return post, err
	}
	logrus.Info("DONE:get post body from psql")
	return post, err
}

func (repo *PostDB) UpdatePostImage(userID, postID int, filePath string, logrus *logrus.Logger) (int64, error) {

	query := fmt.Sprintf("UPDATE %s pl SET post_image_path = $1,updated_at=NOW() FROM %s upl   WHERE pl.id = upl.post_id AND upl.post_author_id = $2 AND upl.post_id = $3 AND pl.deleted_at IS NULL RETURNING pl.id", postTable, postUserTable)
...
func (repo *PostDB) UpdatePost(userID, postID int, post model.UpdatePost, logrus *logrus.Logger) (int64, error) {
	updateQuery := fmt.Sprintf("UPDATE %s pl SET  post_title=COALESCE($1,post_title) ,post_body=COALESCE($2,post_body), post_tags=COALESCE($3,post_tags) , updated_at=NOW() FROM %s upl   WHERE pl.id = upl.post_id AND upl.post_author_id = $4 AND upl.post_id = $5 AND pl.deleted_at IS NULL RETURNING pl.id", postTable, postUserTable)
...
func (repo *PostDB) DeletePost(userID, postID int, logrus *logrus.Logger) (int64, int64, error) {
	...
	deletePostUserQuery := fmt.Sprintf("UPDATE %s SET deleted_at = NOW() WHERE post_id = $1 AND post_author_id=$2 RETURNING id", postUserTable)
	deletePostUserRow, err := repo.db.Exec(deletePostUserQuery, postID, userID)
	...
	logrus.Info("DELETED ID : %v,%v", deletedPost, deletedPostUser)
...
func (repo *PostDB) GetCommitPost(postID int, pagination model.Pagination, logrus *logrus.Logger) (commits []model.CommitFull, err error) {
	query := fmt.Sprintf("SELECT 	cmt.id,		u.firstname, u.secondname,u.nickname,	u.account_image_path,cmt.commits ,cmt.review_data	 FROM %s u INNER JOIN %s cmt on u.id =cmt.reader_id WHERE cmt.post_id = $1 AND cmt.deleted_at IS NULL ORDER BY cmt.review_data DESC OFFSET $2 LIMIT $3 ", usersTable, postCommitTable)
...
func (repo *PostDB) GetResentPost(pagination model.Pagination, logrus *logrus.Logger) (posts []model.PostFull, err error) {
	...
	query := fmt.Sprintf("SELECT p.id , p.post_title ,p.post_image_path, p.post_views_count, p.post_like_count, p.post_rated, p.post_vote_count, p.post_tags, p. post_date, p.is_new, p.is_top_read,pu.post_author_id FROM %s p INNER JOIN %s pu on p.id =pu.post_id WHERE  pu.deleted_at IS NULL AND p.deleted_at IS NULL AND p.deleted_at IS NULL ORDER BY p.post_date DESC OFFSET $1 LIMIT $2 ", postTable, postUserTable)
...
func (repo *PostDB) GetMostViewed(pagination model.Pagination, logrus *logrus.Logger) (posts []model.PostFull, err error) {
	...
	query := fmt.Sprintf("SELECT p.id , p.post_title ,p.post_image_path, p.post_views_count, p.post_like_count, p.post_rated, p.post_vote_count, p.post_tags, p. post_date, p.is_new, p.is_top_read,pu.post_author_id FROM %s p INNER JOIN %s pu on p.id =pu.post_id WHERE  pu.deleted_at IS NULL AND p.deleted_at IS NULL ORDER BY p.post_views_count DESC OFFSET $1 LIMIT $2 ", postTable, postUserTable)
	err = repo.db.Select(&posts, query, pagination.Offset, pagination.Limit)
	if err != nil {
		logrus.Errorf("ERROR: don't get users %s", err)
		return posts, err
...
func (repo *PostDB) GetMostLiked(pagination model.Pagination, logrus *logrus.Logger) (posts []model.PostFull, err error) {
	...
	query := fmt.Sprintf("SELECT p.id , p.post_title ,p.post_image_path, p.post_views_count, p.post_like_count, p.post_rated, p.post_vote_count, p.post_tags, p. post_date, p.is_new, p.is_top_read,pu.post_author_id FROM %s p INNER JOIN %s pu on p.id =pu.post_id WHERE  pu.deleted_at IS NULL AND p.deleted_at IS NULL ORDER BY p.post_like_count DESC OFFSET $1 LIMIT $2 ", postTable, postUserTable)
	err = repo.db.Select(&posts, query, pagination.Offset, pagination.Limit)
	if err != nil {
		logrus.Errorf("ERROR: don't get users %s", err)
		return posts, err
...
func (repo *PostDB) GetMostRated(pagination model.Pagination, logrus *logrus.Logger) (posts []model.PostFull, err error) {
	...
	query := fmt.Sprintf("SELECT p.id , p.post_title ,p.post_image_path, p.post_views_count, p.post_like_count, p.post_rated, p.post_vote_count, p.post_tags, p. post_date, p.is_new, p.is_top_read,pu.post_author_id FROM %s p INNER JOIN %s pu on p.id =pu.post_id WHERE  pu.deleted_at  IS NULL AND p.deleted_at IS NULL ORDER BY p.post_rated DESC OFFSET $1 LIMIT $2 ", postTable, postUserTable)
	err = repo.db.Select(&posts, query, pagination.Offset, pagination.Limit)
	if err != nil {
		logrus.Errorf("ERROR: don't get users %s", err)
		return posts, err
